...
- **Squashed** — Single commit with a message that matches the PR title and body.
- **Designs satisfied** — Declarative designs in `designs/` are not violated by the implementation.
- **Designs updated** — New concepts introduced by this change are captured in designs.
- **Muse approved** — Consult muse (`ellis_ask`) with the diff and relevant context. If the muse has actionable comments, address them and re-consult with the updated diff. Repeat until no actionable comments remain.
